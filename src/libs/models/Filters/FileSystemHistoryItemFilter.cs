...
public class FileSystemHistoryItemFilter : PageFilter
{
    #region Properties
    ...
    #region Constructors
    public FileSystemHistoryItemFilter() { }

    public FileSystemHistoryItemFilter(Dictionary<string, Microsoft.Extensions.Primitives.StringValues> queryParams) : base(queryParams)
    {
    ...
    #region Methods
    public ExpressionStarter<Entities.FileSystemHistoryItem> GeneratePredicate()
    {
        var predicate = PredicateBuilder.New<Entities.FileSystemHistoryItem>();
        if (this.Name != null)
        ...
        if (this.TenantId != null)
            predicate = predicate.And((u) => u.FileSystemItem!.ServerItem!.TenantId == this.TenantId);
        if (this.OrganizationId != null)
            predicate = predicate.And((u) => u.FileSystemItem!.ServerItem!.OrganizationId == this.OrganizationId);
        if (this.OperatingSystemItemId != null)
            predicate = predicate.And((u) => u.FileSystemItem!.ServerItem!.OperatingSystemItemId == this.OperatingSystemItemId);
        if (this.ServerItemServiceNowKey != null)
            predicate = predicate.And((u) => EF.Functions.Like(u.FileSystemItem!.ServerItemServiceNowKey, this.ServerItemServiceNowKey));
