...
public class KeycloakHelper : IKeycloakHelper
{
    ...
    public async Task<Entities.User?> ActivateAsync(ClaimsPrincipal principal)
    {
        ...
        var user = _userService.FindByKey(key.ToString());
        ...
        if (user == null)
        {
            ...
            if (user == null)
            {
                ...
                user = new HSB.Entities.User(username, email, key.ToString())
                {
    ...
    public async Task<UserModel> UpdateUserAsync(UserModel model)
    {
        ...
        if (!String.IsNullOrWhiteSpace(user.Key) && Guid.TryParse(user.Key, out Guid key))
        {
            ...
            if (kUser != null)
            {
                ...
                var roles = model.Groups.SelectMany(g => g.Roles.Select(r => r.Name)).Distinct().ToArray();
                await UpdateUserRolesAsync(key, roles);
            }
        }
    ...
    public async Task DeleteUserAsync(HSB.Entities.User entity)
    {
        ...
        _userService.CommitTransaction();
        if (!String.IsNullOrWhiteSpace(entity.Key) && Guid.TryParse(entity.Key, out Guid key)) await _keycloakService.DeleteUserAsync(key);
    }
    #endregion
}
